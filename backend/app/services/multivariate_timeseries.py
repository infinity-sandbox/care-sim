...
from app.schemas.forcaster_schema import ForecastResponse
...
class MultivariateTimeSeries:
    ...
    TODO:
        ...
        thresholds for every table
        pridicted values for future features of the table
        i need causes table with id referencing each table features (feature selection)
    '''
    @staticmethod
    def connect_db():
        try:
            ...
            config = {
                'user': logger_settings.DB_USER,
                'password': logger_settings.DB_PASSWORD,
                'host': logger_settings.DB_HOST,
                'database': logger_settings.DB,
                'port': logger_settings.DB_PORT
            }
    ...
    async def forecast(column: str, days: int = 1):
        ...
        def calculate_maxlags(num_observations, 
            ...
            try:
                ...
                maxlags = max(1, num_observations // 2)  # Reduce maxlags and retry
    ...
    async def find_issues_based_on_thresholds(df: pd.DataFrame, thresholds: dict) -> dict:
        """
            ...
            thresholds, depending on the threshold type for each column.
